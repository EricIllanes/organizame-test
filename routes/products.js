...
router.post('/product', async(req,res)=>{
    try {
        ...
        const findProductByCategoria= category.find(categories=> categories.id=== categoria)
        let checkNameNotRepeat= product.find(products=> products.nombre_producto === nombre_producto)
        if(!sku || !nombre_producto || !descripcion || !precio || !categoria ){
            res.status(403).send({message:'Debe proporcionar todos los datos'})
        }else if(sku.length >5){
            res.status(403).send({message:'sku no puede tener más de 5 carácteres'})
        } else if(!findProductByCategoria){
          res.status(403).send({message:`No se ha encontrado una categoría con el id: ${categoria}`})
        } else if(checkNameNotRepeat){
          res.status(403).send({message:`Ya existe un producto con el nombre ${nombre_producto}`})
        }else{
                product.push({
                    id: acum,
                    sku,
                ...
                acum++
                res.status(200).send({message:`${nombre_producto} se ha agregado correctamente`})
        }
    } catch (error) {
        res.status(500).send({ message: "Algo ha fallado en el servidor" });
        console.error(error);
...
router.put('/product/:id', async(req,res)=>{
  try {
      const {id}=req.params
    const {sku, nombre_producto, descripcion, precio, categoria} = req.body
    const productForUpdate= product.find(products=> products.id.toString()===id)
    const findProductByCategoria= category.find(categories=> categories.id=== categoria)
    if(!productForUpdate){
        res.status(403).send({message:`No hay productos relacionados al id: ${id}`})
    }else{
          if(sku && sku.length > 5){
            res.status(403).send({message:"Sku no puede tener más de 5 carácteres"})
          } else if(precio && typeof precio !== 'number'){
            res.status(403).send({message:"El precio deben ser números"})
          }else if(!findProductByCategoria ){
            res.status(403).send({message:`No se ha encontrado una categoria con el id: ${categoria}`})
          } else {
          ...
          res.status(200).send({message:`El producto de id: ${id} se ha actualizado correctamente`})
          }
    }
    } catch (error) {
        res.status(500).send({ message: "Algo ha fallado en el servidor" });
...
router.delete('/product/:id', async(req,res)=>{
  try {
    const {id}=req.params
    let position = product.findIndex(products => products.id.toString() === id)
        if(position === -1){
            res.status(403).send({message:`No hay productos asociadas al id: ${id}`})
        } else{
