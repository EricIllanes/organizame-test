...
router.get('/category', async (req,res)=>{
    try {
        ...
        if(nombre_categoria){
            let categoryFindByName = category.find(categories => categories.nombre_categoria.toLowerCase() === nombre_categoria.toLowerCase())
            ...
            let categoryFindById = category.find(categories => categories.id.toString() === id)
            if(categoryFindById){
...
router.post('/category', async(req,res)=>{
    try {
        ...
        if(!nombre_corto.trim() || !nombre_categoria.trim() || !descripcion.trim())  res.status(403).send({message: "Debe proporcionar todos los datos"})
...
router.put('/category/:id', async(req,res)=>{
    try {
        ...
        const categoryForUpdate= category.find(categories=> categories.id.toString()===id)
    if(!categoryForUpdate){
        res.status(403).send({message:`No existe categoría con id: ${id}`})
    } else{
        if(nombre_corto && nombre_corto.length > 5){
            res.status(403).send({message:"nombre_corto no puede tener más de 5 carácteres"})
        } else{
            if(nombre_corto) categoryForUpdate.nombre_corto=nombre_corto
            if(nombre_categoria) categoryForUpdate.nombre_categoria=nombre_categoria
            if(descripcion) categoryForUpdate.descripcion=descripcion
            res.status(200).send({message:`La categoría de id: ${id} se ha actualizado`})
        }
    }
    } catch (error) {
        res.status(500).send({message:'Algo ha fallado en el servidor'})
...
router.delete('/category/:id', async (req,res)=>{
    try {
        const {id} =req.params
        let position = category.findIndex(categories => categories.id.toString() === id)
        if(position === -1){
            res.status(403).send({message:`No hay categorías asociadas al id: ${id}`})
        } else{
